  ...
  group('Verified Badge & Dynamic Oracle Tests', () {
    ...
    test('PetModel.isVerified detects verification prefixes and mint certificates', () {
      final unverifiedPet = PetModel(
        ...
        name: 'Rocky',
        bio: 'Mi perrito',
        avatarUrl: 'https://example.com/rocky.jpg',
        nftMintAddress: null,
      ...
      final standardVerifiedPet = unverifiedPet.copyWith(
        nftMintAddress: 'Verified_${nowMs}_rocky_badge',
      );
    ...
    test('PetModel 40-day expiration deactivates verified badge when older than 40 days', () {
      ...
      final expiredPet = unverifiedPet.copyWith(
        nftMintAddress: 'SolVerified_${expiredMs}_pet_rocky',
      );
      ...
      final activePet = unverifiedPet.copyWith(
        nftMintAddress: 'SolVerified_${activeMs}_pet_rocky',
      );
      ...
      final renewedPet = expiredPet.copyWith(
        nftMintAddress: 'SolVerified_${renewedMs}_pet_rocky',
      );
    ...
    test('Chico is permanently exempted and always holds an active verified badge', () {
      // 1. Chico with null nftMintAddress
      final chicoDefault = PetModel(
        id: 'pet_d1148fad',
        ownerId: 'usr_VL5CBAhr',
        name: 'Chico',
        bio: 'El perrito oficial',
        avatarUrl: 'https://example.com/chico.jpg',
        nftMintAddress: null,
        createdAt: DateTime.now().subtract(const Duration(days: 100)),
      );
      expect(chicoDefault.isExempted, isTrue);
      expect(chicoDefault.isVerified, isTrue);
      expect(chicoDefault.isVerificationExpired, isFalse);
      expect(chicoDefault.verificationDaysRemaining, equals(999));

      // 2. Chico with expired timestamp (> 40 days) -> STILL VERIFIED!
      final expiredMs = DateTime.now().subtract(const Duration(days: 80)).millisecondsSinceEpoch;
      final chicoOld = chicoDefault.copyWith(
        nftMintAddress: 'SolVerified_${expiredMs}_pet_d1148fad',
      );
      expect(chicoOld.isExempted, isTrue);
      expect(chicoOld.isVerified, isTrue);
      expect(chicoOld.isVerificationExpired, isFalse);

      // 3. Static checkVerificationAddress recognises Chico
      expect(PetModel.checkVerificationAddress(null, null, 'Chico'), isTrue);
      expect(PetModel.checkVerificationAddress('SolVerified_old', null, 'chico'), isTrue);
    });
  });
}
